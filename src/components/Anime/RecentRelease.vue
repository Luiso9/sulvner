<template>
  <div class="home-page">
    <h1 class="page-title text-2xl font-bold mb-4">Recent Anime</h1>
    ...
    <div v-else class="anime-list">
      <swiper :slides-per-view="4" :space-between="30" loop>
        <swiper-slide v-for="anime in filteredAnimeList.slice(0, 12)" :key="anime.id" class="anime-card bg-white rounded-lg p-4">
          <div class="anime-image-container w-full h-64 overflow-hidden rounded-md">
            <img :src="anime.image" :alt="anime.title" class="anime-image w-full h-full object-cover" />
          </div>
          <h3 class="anime-title text-lg font-semibold mt-2">{{ anime.title.substring(0,26)+".."  }}</h3>
          <p class="anime-description text-sm text-gray-600">
            Type: {{ anime.type }} - Duration: {{ anime.duration }}
          </p>
          <p class="text-sm text-gray-600">
            Episodes: {{ anime.episodes }} (Sub: {{ anime.sub }}, Dub: {{ anime.dub }})
          </p>
          <div class="mt-2 flex space-x-2">
            <button @click="viewAnimeDetails(anime.id)" class="anime-details-button bg-blue-500 text-white px-3 py-1 rounded-md">
              View Details
            </button>
            <button @click="watchAnime(anime.id)" class="anime-watch-button bg-green-500 text-white px-3 py-1 rounded-md">
              Watch
            </button>
          </div>
        </swiper-slide>
